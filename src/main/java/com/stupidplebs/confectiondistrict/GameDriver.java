...
public class GameDriver {
    public static void main(String[] args) {
        try {
            ...
            final GameSession gameSession = yamlConfigReader.createGameSession(new FileInputStream(new File(filename)));
    ...
    public void run(final Integer numberOfGames, final GameSession gameSession) {
        ...
        for (final Player player : winners.keySet()) {
            System.out.println(player + ": " + winners.get(player) + " ("
                    + (((double) winners.get(player) / (double) numberOfGames)) * 100.0 + "%)");
        }
        ...
        // for (final Map<Player,List<Card>> gameOutcome :
        // gameSession.getGames(10)) {
